<?php
	...
	function isNull($nombre,$apellido,$email,$pw,$cedula,$telefono,$ciudad,$direccion)
	{
		if(strlen(trim($nombre)) < 1 || strlen(trim($apellido)) < 1 || strlen(trim($email)) < 1 || strlen(trim($pw)) < 1 || strlen(trim($cedula)) < 1 || strlen(trim($telefono)) < 1 || strlen(trim($ciudad)) < 1 || strlen(trim($direccion)) < 1)
		{
			return true;
			} else {
			return false;
